...
This is the versioned public-output reference for `figma`.
...
- Intentional text and file artifact outputs remain unchanged unless `--json` explicitly requests an existing compatibility envelope.
...
Result and error envelopes belong on stdout. Progress and debug diagnostics
belong on stderr. Commands do not print raw dependency errors, credentials,
absolute private paths, or stack traces.
...
The no-argument view reports the canonical absolute executable path with home
directory collapsed to `~`. It shows only cheap authentication state and performs
no network requests. See
...
| `assets`, `export` | files and result details | explicit output path and node scope |
...
not retrieve API pages that the underlying endpoint has not supplied, and it does
not override `--depth` or other command-specific bounds.
