...
func TestLayoutCompareCommandUsesExplicitOrderedFrames(t *testing.T) {
	...
	result := executeCommand(newLayoutCompareCommand(DepsForLoadClient(func() (*figma.Client, error) { return client, nil })), "abc", "--id", "1-1", "--id", "2:1")
...
func TestLayoutCompareCommandRequiresTwoFramesBeforeLoadingClient(t *testing.T) {
	loaded := false
	result := executeCommand(newLayoutCompareCommand(DepsForLoadClient(func() (*figma.Client, error) {
		loaded = true
		return nil, nil
	})), "abc", "--id", "1:1")
