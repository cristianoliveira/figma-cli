...
func init() {
	...
	assetsCmd.Flags().String("format", assetFormatAuto, "export format: auto, png, jpg, svg, or pdf")
	assetsCmd.Flags().String("kind", "all", "asset kind: all, image, instance, or vector")
	assetsCmd.Flags().Bool("allow-partial", false, "exit successfully when only some assets export")
