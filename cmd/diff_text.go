...
// newDiffTextCommand constructs `figma diff text` using the explicit
// loadClient dependency. Flag state lives on per-instance variables.
func newDiffTextCommand(deps Deps) *cobra.Command {
  ...
  figma diff text --from <version-id> --to <version-id> --quiet <file-key>`,
	...
	command.Flags().Bool("quiet", false, "suppress output; exit 0 if changes exist, 1 if none (grep-style)")
	return command
}
